...
public static class Utils
{
	...
	public static void DrawCircle(Vector2 pos, float radius, int num_segments, float starting_angle, Color color, float width, float lifetime = 0f, int zIndex = 0, float progress = 0f)
	{
		var step = 2f * MathF.PI / (float)num_segments;
		float zigzagFactor = Utils.Map(progress, 0f, 1f, 0f, 0.25f);
		for(int i = 1; i < num_segments + 1; i++)
		{
			var to_angle = i * step + starting_angle;
			float zz_to = 1f + zigzagFactor * (i % 2 == 0 ? 0f : 1f);
			var to_point = new Vector2(pos.x + radius * zz_to * MathF.Cos(to_angle), pos.y + radius * zz_to * MathF.Sin(to_angle));
